...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('lastActivityAt', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .addColumn('createdAt', 'timestamp', (col) => col.defaultTo(sql`now()`).notNull())
    .execute();

  await db.schema.createIndex('chat_user_id_index').on('chat').column('userId').execute();
  ...
  await db.schema
    .createTable('message')
    .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo(sql`uuid_generate_v4()`))
    .addColumn('chatId', 'uuid', (col) => col.notNull().references('chat.id').onDelete('cascade'))
  ...
  await db.schema.createIndex('message_chat_id_index').on('message').column('chatId').execute();
}
